package cn.gokong.www.base.jwt;
...
public class JwtUtil {
    ...
    public static String buildJWT(SignatureAlgorithm alg, Key key, String sub, String aud, String jti, String iss, Date nbf, Integer duration) {
        ...
        DateTime exp = null;
        if (duration != null)
            exp = (nbf == null ? DateUtil.offsetSecond(iat,duration) : DateUtil.offsetSecond(nbf,duration));
     ...
     * 构建JWT
     * <p>使用 UUID 作为 jti 唯一身份标识</p>
     * <p>JWT有效时间 60*60*24*30 秒，即 30天</p>
     *
     * @param sub jwt 面向的用户
     * @return JWT字符串
     */
    public static String buildJWT(String sub) {
        return buildJWT(sub, null, UUID.randomUUID().toString(), null, null, 60*60*24*30);
    }
    ...
    public static Jws<Claims> parseJWT(Key key, String claimsJws) {
        ...
        //claimsJws = StrUtil.removeAll(claimsJws, JWT_SEPARATOR);
        // 解析 JWT 字符串
    ...
    public static Boolean checkJWT(String claimsJws) {
        ...
        try {
            ...
            StaticLog.error("JWT验证出错，错误原因：{}", e.getMessage());
        }
        return flag;
    }
    ...
    public static void main(String[] args) {
        String token = buildJWT("admin", 60);
        System.out.println(token);
        Boolean admin = checkJWT(token);
